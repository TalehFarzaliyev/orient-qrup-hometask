    ...
    if (isset($_POST['post-type']) and !empty($_POST['post-type']) and $_POST['post-type'] == 'create') {
      ...
      if ($result_insert) {
        ...
        foreach ($translation as $key => $value) {
          ...
          $selected_category[$key] = $value['menu_id'];
        }
                ...
                <div class="row">
                  <div class="col-4 form-section">
                    ...
                    <div class="form-group">
                      <label for="exampleFormControlSelect1" style="margin: 23px 0;">Ana kateqoriya</label>
                      ...
                      <select class="form-control" name="type" id="exampleFormControlSelect1">
                        ...
                        <?php
                          $select_sql  = "SELECT * FROM `menu` m
                                          ...
                                          WHERE  m.parent_id = 0 AND mt.lang_id=1 AND m.type!=''";
                          $result      = mysqli_query($conn, $select_sql);
                          while ($row1   = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                            ?>
                                <option value="<?= $row1['menu_id']; ?>"><?= $row1['type']; ?></option>;
                            <?php
                            }
                            ?>
                  ...
                  <div class="col-8">
                    ...
                    <div class="tab-content" id="nav-tabContent">
                      ...
                      ?>
                        <div class="tab-pane fade show <?php if ($row['code'] == 'az') echo 'active' ?>" id="nav-<?= $row['code']; ?>" role="tabpanel" aria-labelledby="nav-<?= $row['code']; ?>-tab">
                          <div class="form-group">
                            <label for="exampleFormControlInput1" class="form-label" style="margin-top: 20px;">Menyu elementi adı</label>
                            <input type="text" name="translation[<?= $row['id'] ?>][name]" class="form-control" id="exampleFormControlInput1" placeholder="Menyu  adı">
                          </div>
                          <div class="form-group">
                    ...
                    <div class="form-group">
                      <button type="submit" class="btn btn-primary" style="margin-top:5%; margin-left: 40%; margin-right:50%; width: 10em;" value="Submit">Yadda saxla</button>
                    </div>
                  </div>
                </div>
              </form>
            <?php
              ...
              if (!empty($menu_id)) {
                ...
                $sql_tr    =   "SELECT * FROM languages lang
                                INNER JOIN menu_translation mt ON lang.id = mt.lang_id 
                                WHERE mt.menu_id=$menu_id";
                $result    =   mysqli_query($conn, $sql);
                $menu_row  =   mysqli_fetch_assoc($result); //menyu cədvəlindəki datalar götürülür. 
                $result_tr =   mysqli_query($conn, $sql_tr);
                $menu_tr   =   mysqli_fetch_all($result_tr, MYSQLI_ASSOC); //menyu translation cədvəlindəki datalar götürülür.
              }
            ?>
                  ...
                  <div class="col-4 form-section">
                    ...
                    <div class="form-group">
                      ...
                      <select class="form-control" name="parent_id" id="exampleFormControlSelect1">
                        ...
                        $select_sql  = "SELECT * FROM `menu` m
                                        INNER JOIN menu_translation mt ON m.id=mt.menu_id 
                                        WHERE m.type='painter' AND m.parent_id>0 AND mt.lang_id=1";
                        $result      = mysqli_query($conn, $select_sql);
                        while ($row1 = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                        ?>
                    ...
                    <div class="tab-content" id="nav-tabContent">
                      ...
                      ?>
                        <div class="tab-pane fade show <?php if ($value['code'] == 'az') echo 'active' ?>" id="nav-<?= $value['code']; ?>" role="tabpanel" aria-labelledby="nav-<?= $value['code']; ?>-tab">
                          <div class="form-group">
                            <label for="exampleFormControlInput1" class="form-label" style="margin-top: 20px;">Menyu elementi adı</label>
                            <input type="text" class="form-control" name="translation[<?= $value['lang_id'] ?>][name]" value="<?= $value['name']; ?>" id="exampleFormControlInput1" placeholder="Menyu elementinin adı">
                          </div>
                          <div class="form-group">
                            <label for="exampleFormControlInput1" class="form-label">Slug</label>
                            <input type="text" name="translation[<?= $value['lang_id'] ?>][slug]" value="<?= $value['slug']; ?>" class="form-control" id="exampleFormControlInput1">
                          </div>
                        </div>
                      <?php
                      }
