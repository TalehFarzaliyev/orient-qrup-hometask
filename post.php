...
<body>
    ...
    if (isset($_GET['post']) and !empty($_GET['post'])) {
        ...
        $select_post    = "SELECT p.id as id_post, p.image, p.status, pt.* FROM posts p 
    ...
    <div class="portfolio change-theme">
        <h2 class="head-text post-head change-theme portfolio-header profile-header"><?= $post_row['title']; ?></h2>
