<!DOCTYPE html>
<html lang="en">

<?php include 'includes/header.php'; ?>

<body>
    <div id="go-up" class="header">

        <?php include 'includes/menu.php'; ?>

        <div class="menu-name"></div>
    </div>

    <?php include 'includes/theme.php'; ?>

    <div class="portfolio change-theme">
        <h2 class="head-text change-theme portfolio-header profile-header">Haqqında məlumat</h2>
        <div class="icon title-line">
            <img src="assets/img/title-line.png">
        </div>
        <div class="portfolio-pictures">
            <div id="first-category" class="port-picture container img-section">
                <div class="single-img">
                    <img src="assets/img/1.jpg">
                </div>
                <p>Lorem mətn şəhəri ğasəliç ipsum dolor sit amet consectetur adipisicing elit. Molestiae eius pariatur repudiandae architecto ratione rem, voluptatum nisi maxime voluptate quod quo fuga dolores quaerat optio voluptatibus distinctio iste
                    recusandae qui amet ex. Delectus sit magni nihil magnam vel accusantium nisi illum quod ipsa commodi laborum assumenda minima minus, architecto quas fugit quo! Hic, velit iste dolore voluptatibus excepturi quia corporis sequi dicta.
                    Harum natus quo dolor aliquam. Voluptatem, nam dolore dolor modi tempora aspernatur consequatur nulla aut repudiandae id numquam perferendis eum sed ab nesciunt repellat est pariatur vero ipsum quas! Laudantium est id unde, repudiandae
                    odit ea possimus hic minima perspiciatis ratione suscipit ad sequi voluptates recusandae officia ut porro? Excepturi nisi reprehenderit enim beatae, rerum est a labore mollitia? Porro repellat veniam voluptatum distinctio recusandae
                    debitis quae sequi voluptatem eius aliquam. Ipsam repellendus accusamus minus doloremque. Accusantium quos ut similique soluta. Voluptates, quibusdam amet facere nobis provident voluptatem fuga inventore harum nesciunt. Quaerat similique
                    repellendus hic possimus aspernatur numquam ut sed asperiores, aperiam vel vitae voluptas eligendi reiciendis magni est accusamus quibusdam labore inventore odit amet at illo id tenetur? Fugit, iste. Vitae illo enim accusamus officiis
                    officia. Explicabo dolor reprehenderit qui eum iusto voluptates earum officiis necessitatibus.</p>
            </div>
        </div>
    </div>

    <?php include 'includes/footer.php'; ?>

</body>

</html>