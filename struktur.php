...
<body>
    ...
    $number_of_content = mysqli_num_rows(mysqli_query($conn, "SELECT `id` FROM `posts` p INNER JOIN menu_translation mt ON mt.menu_id=p.category_id WHERE mt.name='struktur'"));
    $limit = 4;
    ...
    if ($lastpage >= $page) {
        $result = mysqli_query($conn, "SELECT p.id as id_post, p.image, p.created_date, p.category_id, pt.*, mt.*, m.* FROM orient_ressamlar.posts p 
                                   ...
                                   WHERE pt.lang_id=$lang_id && p.status=1 && p.category_id=m.id && mt.name='struktur' ORDER BY p.`id` desc LIMIT " . $start . ',' . $limit);
        $post  = mysqli_fetch_all($result, MYSQLI_ASSOC);
        if (!empty($post)) {
    ?>

            <div class="portfolio change-theme">
                <h2 class="head-text change-theme portfolio-header profile-header"><?php if ($lang_id == 1) {
                                                                                        echo 'Struktur';
                                                                                    } else echo 'Structure'; ?></h2>
